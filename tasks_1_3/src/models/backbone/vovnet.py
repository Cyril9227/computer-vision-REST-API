"""
Defines a VoVNet backbone

Forked from https://github.com/youngwanLEE/vovnet-detectron2/
...
class VoVNet(Backbone):
    def __init__(self, cfg, input_ch, out_features=None):
        ...
        input_ch ([type]): The number of input channel
        out_features (list[str], optional): Name of the layers whose outputs should
            be returned in forward. Can be anything in "stem", "stage2" .... Defaults to None.
        """
...
def build_vovnet_backbone(cfg, input_shape):
    """Create a VoVNet instance from config. The type of network (mobilenet-like or deeper model) is defined by the CONV_BODY config entry.
